    ...
    for(int j = 0; j < n_width - 1; ++j) {
        ...
        for (int j = 0; j < n_height; ++j) {
            for (int i = 0; i < n_width; ++i) {
                points.push_back(Vec3(i / (float)n_width, j / (float)n_height, z));
...
void drawTerrain() {
    ...
    Vec3 look = cameraFront + cameraPos;
    Mat4x4 V = lookAt(Vec3(0.5, 0, 1.0), look, Vec3(0, 1, 0));
...
int main(int, char**) {
    ...
    /*window.add_listener<KeyEvent>([&](const KeyEvent& k) {
        ...
        //right (GLFW_KEY_D)
        if (k.key == GLFW_KEY_D) {
            cameraPos = cameraPos + speed * cameraFront.cross(cameraUp.normalized()).normalized();
        }
        //left(GLFW_KEY_A)
        if (k.key == GLFW_KEY_A) {
            cameraPos = cameraPos + speed * -cameraFront.cross(cameraUp.normalized()).normalized();
